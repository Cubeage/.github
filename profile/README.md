<!--
  Org profile: https://github.com/Cubeage
  (Root README.md remains CI / Actions infrastructure docs.)
  Copy SSOT: https://github.com/Cubeage/brand/docs/copy
-->

<img src="https://mark.sylphx.com/api?type=blur&height=220&color=0:A02028,50:E03840,100:1A1A1A&text=Cubeage&fontSize=52&fontColor=ffffff&animation=twinkling&desc=Classic%20card%20games.%20Reimagined%20for%20mobile.&descSize=16&descAlignY=62&fontAlignY=42" width="100%" alt="Cubeage"/>

<p align="center">
  <b>Premium mobile board &amp; card games · Hong Kong</b><br/>
  Authentic regional rules · Crafted tables · Free to play
</p>

<p align="center">
  <a href="https://cubeage.com"><img src="https://mark.sylphx.com/badge/Website-cubeage.com-E03840?style=for-the-badge" alt="Website"/></a>
  <a href="https://apps.apple.com/developer/cubeage-limited/id1238318955"><img src="https://mark.sylphx.com/badge/App%20Store-000000?style=for-the-badge&logo=apple&logoColor=white" alt="App Store"/></a>
  <a href="https://play.google.com/store/apps/dev?id=7470180929763617319"><img src="https://mark.sylphx.com/badge/Google%20Play-414141?style=for-the-badge&logo=googleplay&logoColor=white" alt="Play"/></a>
  <a href="https://github.com/Cubeage/brand"><img src="https://mark.sylphx.com/badge/Brand-SSOT-A82828?style=for-the-badge" alt="Brand"/></a>
</p>

---

## What we build

**Cubeage** publishes **premium board and card games** — mahjong, Big 2, and related classics — with **authentic Hong Kong & Taiwan rules**, live multiplayer, and production craft.

| Series | Intent |
|--------|--------|
| **Fun** | Fast social tables — competitive, light session |
| **Tycoon** | Progression, characters, seasons — deeper empires on real rules |

> Rules first. Craft always. Free to start.

---

## Catalog (highlights)

- Mahjong Tycoon (HK / TW)  
- Big 2 Tycoon family  
- Fun Big 2 (HK / TW) · Fun Mahjong 16 · and more  

Full list: **[cubeage.com](https://cubeage.com)**

---

## Engineering on GitHub

Private and public game backends, clients, and shared platform live under this org — Unity, native SDKs, live-ops services, and CI for iOS builds.

Org-wide Actions / macOS runner docs: see root [`README.md`](https://github.com/Cubeage/.github/blob/main/README.md) (infrastructure).

---

## Brand

| | |
|--|--|
| **Legal** | Cubeage Limited |
| **Mark** | Cube + **CUBEAGE** |
| **Support** | support@cubeage.com |
| **SSOT** | [github.com/Cubeage/brand](https://github.com/Cubeage/brand) |

---

<div align="center">

**Classic card games. Reimagined for mobile.**

[cubeage.com](https://cubeage.com) · Hong Kong · Founded 2014

</div>

<img src="https://mark.sylphx.com/api?type=waving&color=0:E03840,100:A02028&height=100&section=footer" width="100%" alt="footer"/>
